...
def test_overview_prompt_does_not_force_pricing_or_money_keywords():
    ...
    assert "MONEY" not in user_prompt
    assert "Include at least two specific dollar amounts." not in user_prompt
    assert "BUSINESS MODEL" not in user_prompt
    assert "specific pricing mechanism" not in user_prompt
    assert "BUSINESS MODEL: Competitor price benchmarks with actual dollar amounts" not in system_prompt
