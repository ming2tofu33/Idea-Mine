...
def test_build_kernel_set_uses_deterministic_defaults_for_missing_core_fields():
    seed = NormalizedSeed(
        actors=[],
        tensions=[],
        outcomes=[],
    ...
    kernel_set = build_kernel_set(seed)
    assert kernel_set.primary_kernel.primary_actor == "user"
    assert kernel_set.primary_kernel.primary_tension == "friction"
    assert kernel_set.primary_kernel.primary_outcome == "better result"
    assert kernel_set.primary_kernel.primary_environment is None
    assert kernel_set.primary_kernel.text == (
        "A user wants better result by addressing friction"
    )
