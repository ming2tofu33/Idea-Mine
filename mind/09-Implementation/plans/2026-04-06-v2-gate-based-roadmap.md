...
- [ ] Free/Lite/Pro 차이 표시 (일일 한도, 감정 깊이, 풀 개요, MVP 청사진)
- [ ] 프라이싱 페이지

**업셀 미리보기 UI (핵심 — 전환율 직결):**

- [ ] 풀 개요 미리보기: Free 유저가 Lab에서 "풀 개요" 버튼 → 블러된 샘플 + "Lite에서 월 3회 생성" CTA
- [ ] MVP 청사진 미리보기: Free/Lite 유저가 "MVP 청사진" 버튼 → 블러된 샘플 + "Pro에서 무제한 생성" CTA  
- [ ] Phase 로드맵 미리보기: 동일 패턴
- [ ] 잠금 기능이 "없는 것"이 아니라 "있는데 잠겨있는 것"으로 디자인
- [ ] 각 미리보기에 실제 샘플 콘텐츠 1개 포함 (이 아이디어의 실제 결과 미리보기가 아닌, 범용 샘플)
- [ ] 감정 depth 업셀: basic_free(3축) 결과 아래에 "6축 정밀 감정은 Lite에서" 안내
...
**MVP 청사진 (Pro only) — 바이브 코딩 핵심 문서:**

- [ ] MVP 청사진 생성 파이프라인 (풀 개요 기반)
  - Must/Should/Later 기능 구체화
  - 기술 스택 확정 + 선택 이유
  - 검증 포인트 (핵심 가설 + 최소 테스트)
  - Sprint 단위 구현 순서 (Phase 0 → 1 → 2)
- [ ] MVP 청사진 표시 UI
- [ ] 마크다운 복사 (AI 코딩 도구에 바로 붙여넣기)

> 풀 개요서 = "이 제품이 뭔지" / MVP 청사진 = "어떤 순서로 만들지"
> 바이브 코딩 사용자가 돈 내고 살 진짜 이유 = 이 문서
