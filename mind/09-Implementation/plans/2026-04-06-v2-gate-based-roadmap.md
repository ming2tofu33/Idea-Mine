...
### Sprint 2: The Vault + The Lab + 어드민 MVP
...
**어드민 MVP (Gate 1 측정 인프라):**

> `ai_usage_logs` 테이블에 데이터가 이미 쌓이고 있으므로 프론트엔드만 구축하면 된다.
> 어드민은 `/admin` 라우트, `role='admin'`일 때만 접근 가능.

- [ ] 어드민 레이아웃: 사이드바 + 메인 콘텐츠 (기존 앱 셸과 분리)
- [ ] 어드민 접근 제어: `profiles.role === 'admin'` 체크, 미들웨어 보호

**비용 대시보드:**

- [ ] 일별 AI 비용 추이 차트 (`ai_usage_logs` → 일별 `total_cost_usd` 합산)
- [ ] 기능별 비용 분류 (채굴 / 개요서 / 감정서 / 풀 개요서)
- [ ] 티어별 비용 분포 (free / lite / pro)
- [ ] L4 한도 대비 소비율 (일일 예산 소진 현황)
- [ ] 에러 로그 목록 (`status='error'` 필터)

**퍼널 분석 (Gate 1 측정용):**

- [ ] 핵심 퍼널 전환율: 광맥 조회 → 채굴 → 금고 반입 → 개요서 생성
- [ ] 일별 요청 수 추이 (기능별)
- [ ] 기능별 완주율 계산

**사용자 기본:**

- [ ] 사용자 목록 (닉네임, 티어, 마이너 레벨, 스트릭, 마지막 활동일)
- [ ] 티어 분포 차트 (free / lite / pro 비율)
- [ ] 개별 사용자 상세 (오늘 사용량, AI 비용 이력)

**기존 AdminFab 이관:**

- [ ] 페르소나 전환을 어드민 사이드바로 이동
- [ ] 일일 상태 리셋 / 광맥 재생성을 어드민 도구 페이지로 이동

### Gate 1: 핵심 루프 동작 검증

**측정 방법:** 5~10명 테스터에게 웹 제품 공유 + 어드민 퍼널 대시보드로 수치 확인
...
> Gate 2는 D1 재방문율 > 20%, 전환율 > 30%, 주간 2회+ 채굴 > 40%를 요구한다.
> Phase 1 어드민 MVP 위에 리텐션/키워드 운영 도구를 쌓는다.

- [ ] DAU / MAU 차트 (일별 활성 사용자 추이)
- [ ] D1 / D7 재방문율 차트 (코호트 기반)
- [ ] 스트릭 분포 히스토그램 (연속 접속일 분포)
- [ ] 주간 활성 사용자 중 2회+ 채굴 비율 추적

**키워드 관리:**

> 현재 118개 키워드는 시드 데이터로 고정. 운영하면서 가중치 조절과 비활성화가 필요해진다.

- [ ] 키워드 목록 (카테고리별 필터, 검색)
- [ ] 가중치(weight) 슬라이더 조정 — 특정 키워드 출현 빈도 조절
- [ ] 활성/비활성 토글 (is_active) — 문제 키워드 즉시 제거
- [ ] 프리미엄 플래그(is_premium) 관리
- [ ] 인기 키워드 조합 Top 10 (가장 많이 채굴된 조합 통계)

**공지사항 시스템:**

- [ ] notices 테이블 생성 (title, content, target_tier, start_date, end_date)
- [ ] 공지 CRUD 어드민 UI
- [ ] 인앱 배너/모달 연동 (사용자 측)
...
**어드민 수익 분석 (Gate 3 측정용):**

> Gate 3는 Free→Pro 전환율 > 2%, Pro 유지율 > 15%를 요구한다.
> 결제 데이터와 티어 전환 이벤트를 추적해야 한다.

- [ ] 티어 전환 이벤트 추적 (free→lite, free→pro, lite→pro, 다운그레이드)
- [ ] 전환율 차트 (일별/주별 Free→Pro 전환)
- [ ] Pro 체험 시작/만료/유지 추적
- [ ] 구독 수익 대시보드 (MRR, 활성 구독 수, 이탈률)
- [ ] 업셀 터치포인트별 전환율 (어떤 블러 미리보기가 가장 효과적인지)

**프롬프트 관리:**

> 프롬프트가 Python 파일에 하드코딩되어 있어 변경 시 재배포 필요.
> 운영 단계에서는 코드 배포 없이 프롬프트를 교체할 수 있어야 한다.

- [ ] 프롬프트 버전 테이블 생성 (feature_type, version, content, is_active)
- [ ] 프롬프트 목록 / 편집 어드민 UI
- [ ] 버전 히스토리 (변경 이력 추적)
- [ ] A/B 테스트 기초 (같은 feature_type에 2개 버전 활성화 → 비율 분배)

**남용 모니터링:**

- [ ] L1 속도 제한 히트 로그 대시보드 (누가 언제 제한에 걸렸는지)
- [ ] 의심 패턴 하이라이트 (짧은 시간 반복 리롤, 비정상 요청 패턴)
- [ ] 사용자 경고/정지 기능 (is_suspended 플래그 + 어드민 UI)

### Gate 3: 수익화 검증

**측정 방법:** 100명+ 사용자 기반 전환 데이터 + 어드민 수익 대시보드로 수치 확인
...
- [ ] 시즌/이벤트 관리 (`active_seasons` 테이블 CRUD + 시즌별 특별 키워드)
- [ ] 피드백 수집 시스템 (원석 품질 / 감정서 품질 사용자 반응)
- [ ] 시스템 헬스 대시보드 (API 응답 시간, 에러율, 업타임)
- [ ] 어드민 액션 감사 로그 (누가 언제 어떤 어드민 조작을 했는지)
- [ ] MCP 소스별 사용량/비용 분석 (`ai_usage_logs.source` 기반)
