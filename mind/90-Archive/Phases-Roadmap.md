...
- [ ] 오늘의 광맥 3개 생성 로직 (백엔드)
- [ ] 광맥 카드 UI (캐러셀)
- [ ] **광맥 카드 기본 프레임** (일반/금빛/전설 3종 — 최소 픽셀)
- [ ] **확률 고지 UI** (광맥 화면 물음표(?) 아이콘 → 등급별 확률표 표시 + 설정 > 확률 정보)
- [ ] **키워드 칩** (카테고리별 컬러 매핑)
- [ ] 리롤 (무료 2회)
- [ ] **Haptic 피드백** (리롤, 채굴)
...
- [ ] 감정 API 라우터 연결 (`appraisal.py` + `appraisal_service.py` + `appraisal.ts` 준비 완료 → 라우터 + 인증/티어 체크 + rate limiting 붙이기)
- [ ] 기본 감정(Appraisal) UI — 점수 아님, **6축 코멘트 카드** (시장성/문제적합/실행/차별화/확장/리스크)
- [ ] 개요서 저장 -> 금고 연결
- [ ] 개요서 텍스트 복사/내보내기
