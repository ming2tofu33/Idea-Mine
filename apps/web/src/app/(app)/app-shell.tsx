        ...
        <StatusRail
          ...
          left={
            <nav className="flex items-center gap-1.5">
              {NAV_ITEMS.map(({ href, label }) => {
                ...
                // 게스트는 /mine 외엔 잠금 표시 (클릭 시 sign-in으로)
                const isLockedForGuest = isGuest && href !== "/mine";
                const linkHref = isLockedForGuest
                  ? `/auth/sign-in?next=${href}`
                  : href;
                return (
                  <Link
                    key={href}
                    href={linkHref}
                    className={[
                      "relative inline-flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-sm tracking-wide transition-all duration-200",
                      isActive
                  ...
                  >
                    {label}
                    {isLockedForGuest && (
                      <Lock className="h-3 w-3 text-text-secondary/50" />
                    )}
                    {isActive && (
          ...
          right={
            <div className="flex items-center gap-2">
              {isGuest ? (
                <span className="hidden rounded-full border border-cold-cyan/30 bg-cold-cyan/10 px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.22em] text-cold-cyan sm:inline-block">
                  Demo mode
                </span>
              ) : (
                <button
